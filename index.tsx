...
// My Page
const renewAuthBtn = document.getElementById('renew-auth-btn') as HTMLButtonElement;
const loginBtn = document.getElementById('login-btn') as HTMLButtonElement;
const logoutBtn = document.getElementById('logout-btn') as HTMLButtonElement;
const infoName = document.getElementById('info-name') as HTMLSpanElement;
const infoDepartment = document.getElementById('info-department') as HTMLSpanElement;

// Login Modal
const loginModal = document.getElementById('login-modal') as HTMLDivElement;
const loginModalCloseBtn = document.getElementById('login-modal-close-btn') as HTMLButtonElement;
const loginModalOverlay = loginModal.querySelector('.modal-overlay') as HTMLDivElement;
const loginForm = document.getElementById('login-form') as HTMLFormElement;
const loginNameInput = document.getElementById('login-name') as HTMLInputElement;
const loginDepartmentInput = document.getElementById('login-department') as HTMLInputElement;
...
async function openAlertModal(alertSummary: string, alertTimestamp: string) {
    ...
    try {
        ...
        const schema = {
          type: Type.OBJECT,
          properties: {
            user: { type: Type.STRING, description: '이벤트와 관련된 사용자입니다.' },
            action: { type: Type.STRING, description: '탐지된 행동에 대한 간결한 설명입니다.' },
            cwe: { type: Type.STRING, description: '해당하는 경우 가장 관련성 높은 CWE(Common Weakness Enumeration) 번호입니다. 해당 없으면 "N/A"를 사용하세요.' },
            risk: { type: Type.STRING, description: '위험 수준으로, "High", "Medium", "Low" 중 하나여야 합니다.' },
            details: { type: Type.STRING, description: '이 이벤트가 잠재적 위험인 이유에 대한 간략한 설명입니다.' },
            recommendation: { type: Type.STRING, description: '보안팀이 이 경고를 처리하기 위한 명확하고 실행 가능한 단계별 권장 조치입니다. 목록에는 마크다운을 사용하세요.' },
          },
        ...
        const response = await ai.models.generateContent({
           ...
           contents: `당신은 사이버 보안 분석 AI입니다. 다음 보안 경고 요약을 분석하고 상세 보고서를 한국어로 작성해주세요. 경고: "${alertSummary}"`,
           config: {
...
/** Closes the login modal and clears the form. */
...
function handleLogout() {
    ...
    infoName.textContent = '홍길동';
    infoDepartment.textContent = '내부통제팀';
